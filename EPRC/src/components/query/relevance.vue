<template>
  <div class="relevance">
    <div class="search_cont">
      <div class="search">
        <input v-model="account1" type="text" placeholder="输入查询账户名1" />
        <input v-model="account2" type="text" placeholder="输入查询账户名2" />
        <button @click="selectrInfo">查询</button>
      </div>
    </div>
    <div class="single_cont" v-show="ShowEchart">
      <div class="title">
        <div class="account">
          <span>关联记录</span>
          <div>火币</div>
        </div>
        <span @click="changeShowEchart">图谱展示</span>
      </div>
      <div class="pull_cont">
        <div class="sinister">
      ...
      <div class="table">
        ...
        <div v-for="(item,index) in tabList" :key="index" style="position: relative;">
          <ul class="tabs_cont bg-g" v-if="index % 2 == 0">
            ...
            <li class="three">{{item.account2}}</li>
            <li :class="item.type=='input'?'four yellow':'four'">{{item.price}}</li>
    ...
    <div class="outCharts" v-show="!ShowEchart" @click="changeShowEchart">退出图谱展示</div>
    <div id="EchartBox" class="single_cont" v-show="!ShowEchart"></div>
  </div>
</template>

<script>
import { collectPost, trackGet, selectr, trackPost } from "../../api/api";
import { Message, Loading } from "element-ui";
export default {
  data() {
    return {
      account1: "e2aa14033b93d65077bad0c2ba67aa9f",
      account2: "7fd3bb968d05d42ad8c96d604745f84f",
      name: "btc",
      sum: "金额",
      rank: "排序",
      time: " 时间",
      ...
      tabList: [
        ...
        //   id: 1,
        //   type: "output",
        //   account1: "0xc97a4ed29f03fd549c4ae79086673523122d2bc5",
        //   account2: "0xc97a4ed29f03fd549c4ae79086673523122d2bc7",
        //   price: "921.79",
        //   time: "2020-03-07 11:04:10",
        //   isBoxes: false
        // }
      ],
      chartData: {}
 ...
 if(JSON.stringify(query) !== '{}'){
    ...
    console.log(query);
    that.account1 = query.account1;
    that.account2 = query.account2;
  ...
  methods: {
    ...
    selectrInfo() {
      ...
      if (this.account1 != "" && this.account2 != "") {
        ...
        if (that.isSum == 0) {
          data = {
            params: {
              account1: that.account1,
              account2: that.account2,
              chain: that.name,
          ...
          data = {
            params: {
              ...
              start_time: that.startTime,
              end_time: that.endTime,
              start_piece: that.startInterval,
              end_price: that.endInterval
            }
          };
        }

        selectr(data).then(res => {
          console.log(res);
          let listdata = res.normal_data;
          let table_data = res.table_data;
          for (let i in listdata) {
            listdata[i].isBoxes = false;
          }
          that.tabList = listdata;
          that.chartData = table_data;
        });
      } else {
        Message({
          message: "填写账户信息",
    ...
    changeShowEchart() {
      ...
      if (!this.ShowEchart) {
        ...
        let datas = [];
        let links = [];
        that.chartData.Data1.forEach((item, index) => {
          console.log(item);
          obj = {};
          obj.id = item.id;
          obj.name = item.hash;
        ...
        links = that.chartData.links;
        that.list = datas;
        console.log(datas);
        console.log(that.chartData.links);
    ...
    echartRun(datas, links) {
      ...
      let dom = document.getElementById("EchartBox");
