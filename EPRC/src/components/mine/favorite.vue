  ...
  <div class="favorite">
    ...
    <div class="box_cont">
      <div class="box" v-for="(F,index) in boxList" :key="index">
        <div class="sinister">
          <div class="account">
            <span v-if='F.type==4' @click="goHistory(index)">{{F.collect_info.chain}}</span>
            <span v-else  @click="goHistory(index)">{{F.collect_info.account_hash}}</span>
...
<script>
import { collectGet,collectPut } from "../../api/api";
...
export default {
  ...
  methods: {
    onDel(i) {
      ...
      collectPut({
        user_id: window.localStorage.getItem("user_id"),
        collect_id: that.boxList[i].collect_id
      }).then(res => {
        console.log(res);
    ...
    goHistory(index){
       let that = this;
      let his=that.boxList[index].collect_info
      console.log(his)
