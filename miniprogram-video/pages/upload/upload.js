...
Page({
  ...
  new_submit: function() {
    ...
    // valueActivity: "",
    // valueAstrict: "",
    console.log(that.data.valueName)
    if (!that.data.valueName){
    ...
    } else if (!that.data.valueTotal){
      wx.showModal({
        title: '提示',
        content: '请填写课程总费用'
      })
    } else if (!that.data.valuePrice) {
      wx.showModal({
        title: '提示',
        content: '请填写报名定金'
      })
    } else if (!that.data.valueActivity) {
      wx.showModal({
        title: '提示',
        content: '请填写报名定金活动价'
      })
    } else if (that.data.valueActivity<3) {
      wx.showModal({
        title: '提示',
        content: '定金活动价最低为三元'
      })
    } else if (!that.data.valueAstrict) {
      wx.showModal({
        title: '提示',
        content: '请填写报名人数上限'
      })
    } else if (!that.data.chargeStatement) {
      wx.showModal({
        title: '提示',
        content: '请填写课程收费说明'
      })
      ...
      // wx.showModal({
      //   title: '请确认',
      //   content: '课程名称'+that.data.valueName,
      // })
      const photos = []
      ...
      console.log(data)
      if (app.globalData.institution_id) {
  ...
  qrcode: function (that, activity_id) {
    wx.request({
      url: 'https://www.heiyunworld.com:12123/wechat/token?type=access_token&secret=1234567890',
      header: {
      ...
      success(res) {
        ...
        wx.request({
          ...
          success(qrcode) {
            ...
            // const filePath
            console.log(filePath,'1111111111111111111111111')
              ...
              success() {
                wx.getImageInfo({
                  ...
                  success: function (res) {
                    ...
                    console.log(res.path,'2222222222222222222222')
                    that.setData({
                      imageEwm: res.path
                    })
